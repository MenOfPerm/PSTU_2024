...
using namespace std;
 ...
 { 
	 ...
	 { charTable[i] = -1; }
	 // заполняем таблицу индексом последнего вхождения этого символа в подстроке 
 ...
 void boyerMooreSearch(string str, string pat) {
	 ...
	 while (shift <= (strSize - patSize)) {
		 int j = patSize - 1; // суффиксный поиск 
		 while (j >= 0 && pat[j] == str[shift+j]) 
		 { j--; }

		 if (j < 0) 
		 {
			 cout << "Подстрока найдена на позиции: " << shift << endl; return;
		 ...
		 else { shift += max(1, j - shiftTable[(int)str[shift + j]]); }
	 }
 }
 int main() {
	 setlocale(LC_ALL, "RU");
